...
- [ ] Clicking the dots while pinned raises the panel and pulses its edge twice instead of doing nothing; if the panel is fully off-screen it returns under the dots first
- [ ] Right-click on the dots: Show Tour, Keyboard Shortcuts, Settings…, Pin Panel Open (checkmark), Bring Panel to the Menu Bar, Reset Panel Position and Size, Open at Login (checkmark), Quit Stoplight
...
- [ ] **Medium widget:** up to 4 PR rows mirroring the popover (sections, order, dot, `#num` (repo name added only when a section spans repos), title). **Large:** 12 rows. Tapping a row opens the panel with that PR selected and expanded (`stoplight://pr/<id>`); the small widget opens the panel (`stoplight://open`)
...
- [ ] Selected PR: ⌘K open the Actions run summary (falls back to the checks tab), ⌘C copy URL, ⇧⌘C share, ⌘B copy branch, ⌘P pin, ⌘F fix with agent, ⌘H hide
- [ ] Expanded row has a run-summary circle button (`…/actions/runs/<id>`, derived from the first failing job's URL) and ends its checks list with "All N checks · M failed ↗" linking to the checks tab; both in the right-click menu
...
- **App → widget channel:** the app serves the snapshot JSON on `127.0.0.1:47391`; the widget fetches it (sandbox `network.client`, no profile needed). No file at all: App Group container writes were observed to block the main thread indefinitely on macOS 15 for this unsandboxed app, and the widget's own container trips the "access data from other apps" prompt. No Core Data, no SwiftData
