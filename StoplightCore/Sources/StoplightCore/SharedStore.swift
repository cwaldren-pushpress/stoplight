...
/// Where the snapshot lives: served by the app on loopback (`SnapshotServer`), fetched by the widget.
///
/// There is deliberately no file. App Group containers need the group in the provisioning profile and,
/// on macOS 15, `FileManager.containerURL` / writes into them can block the calling thread indefinitely
...
public enum SharedStore {
    ...
    /// Widget side: ask the running app. nil when it isn't running.
    public static func load() async -> Snapshot? {
        var req = URLRequest(url: loopbackURL)
        req.timeoutInterval = 2
        guard let (data, resp) = try? await URLSession.shared.data(for: req),
              (resp as? HTTPURLResponse)?.statusCode == 200, let snap = decode(data) else {
            log.notice("load: app not reachable on loopback")
            return nil
        }
